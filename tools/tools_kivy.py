"""
Module tools kivy of Tramway Collector
"""
...
class TutorialPopup():
    """
    Class to display a succession of improved popups, like for a tutorial.
    It takes as input the argument list_popups which is a list composed of dict_popup.
    dict_popup is a dictionary with the following keys:
        - "title": string corresponding to the title of the popup
        - "message": string corresponding to the text displayed in the popup
        - "image": string corresponding to the path of the image to display.
        It may be not set, and thus will take the default value of a blank image.
        - "left_button": string corresponding to the message displayed in the left button.
        It may be not set, and thus will take the default value of "Back".
        - "right_button": string corresponding to the message displayed in the right button.
        It may be not set, and thus will take the default value of "Next".
    """

    def __init__(self, list_popups, font) -> None:
        self.list_popups = list_popups
        self.counter_popup = 0
        self.font = font
        self.init_dict_popups()
        self.build_tutorial()

    def init_dict_popups(self):
        for counter_popup in range(len(self.list_popups)):
            dict_popup = self.list_popups[counter_popup]
            text_tutorial = my_language.dict_language["settings"]["tutorial"]
            if "left_button" not in dict_popup:
                self.list_popups[counter_popup]["left_button"] = text_tutorial["left_button"]
            if "right_button" not in dict_popup:
                self.list_popups[counter_popup]["right_button"] = text_tutorial["right_button"]
            if "image" not in dict_popup:
                self.list_popups[counter_popup]["image"] = BLANK_IMAGE_SOURCE

    def build_tutorial(self):
        dict_popup = self.list_popups[0]
        self.popup = ImprovedPopup(
            title=dict_popup["title"],
            font=self.font
        )
        self.label = self.popup.add_label(
            text=dict_popup["message"],
            size_hint=(0.8, 0.6),
            pos_hint={"x": 0.1, "y": 0.3},
            font_name=self.font
        )
        # Add the image if necessary
        self.image = self.popup.add_other_widget(
            widget_class=Image,
            source=dict_popup["image"],
            allow_stretch=True,
            pos_hint={"center_x": 0.5, "y": 0.3},
            size_hint=(0.8, 0.3)
        )
        if dict_popup["image"] != BLANK_IMAGE_SOURCE:
            self.label.pos_hint["y"] = 0.6
            self.label.size_hint_y = 0.3
        else:
            self.popup.layout.remove_widget(self.image)
        self.left_button = self.popup.add_button(
            text=dict_popup["left_button"],
            size_hint=(0.3, 0.1),
            pos_hint={"x": 0.15, "y": 0.1},
            font_name=self.font
        )
        self.left_button.on_release = self.go_to_previous
        self.right_button = self.popup.add_button(
            text=dict_popup["right_button"],
            size_hint=(0.3, 0.1),
            pos_hint={"right": 0.85, "y": 0.1},
            font_name=self.font
        )
        self.right_button.on_release = self.go_to_next

    def actualise_popup(self):
        dict_popup = self.list_popups[self.counter_popup]
        self.popup.title = dict_popup["title"]
        self.label.text = dict_popup["message"]
        self.image.source = dict_popup["image"]
        ...
        if dict_popup["image"] != BLANK_IMAGE_SOURCE:
            ...
            try:
                ...
                pass
            self.popup.layout.add_widget(self.image)
        else:
            self.label.pos_hint["y"] = 0.3
            self.label.size_hint_y = 0.6
            self.popup.layout.remove_widget(self.image)

        self.left_button.text = dict_popup["left_button"]
        self.right_button.text = dict_popup["right_button"]

    def go_to_previous(self):
        self.counter_popup -= 1
        if self.counter_popup < 0:
            self.popup.dismiss()
            return
        self.actualise_popup()

    def go_to_next(self):
        self.counter_popup += 1
        if self.counter_popup >= len(self.list_popups):
            self.popup.dismiss()
            return
        self.actualise_popup()
